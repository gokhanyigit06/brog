...
// Sabit alt bant + sohbet balonu gibi overlay'leri gizle (kozmetik, sadece kapak için)
await p.evaluate(() => {
  ...
  document.querySelectorAll("div,section,aside").forEach((el) => {
    ...
    const isBottomBar = r.bottom > vh - 8 && r.height < 260 && r.width > innerWidth * 0.5;
    const isBubble = r.bottom > vh - 120 && r.width < 480 && r.left > innerWidth * 0.6;
    if (isBottomBar || isBubble) el.style.display = "none";
  });
});
await p.waitForTimeout(600);
await p.evaluate(() => window.scrollTo(0, 0));
await p.waitForTimeout(500);
await p.screenshot({ path: "scripts/np_woodiko_hero3.jpg", type: "jpeg", quality: 92 });
await b.close();

const buffer = fs.readFileSync("scripts/np_woodiko_hero3.jpg");
...
const blocks = (snap.data().blocks || []).map((bl, i) => (i === 0 && bl.type === "single_image") ? { ...bl, url } : bl);
await withRetry(() => updateDoc(doc(db, "projects", ID), { imageUrl: url, blocks, updatedAt: new Date().toISOString() }));
console.log("✅ Woodiko hero (temiz) güncellendi");
process.exit(0);
