...
// Referans bölümü
const t = await p.locator("text=Fulya Kale").count();
console.log("Fulya Kale görünüyor:", t > 0);
await p.locator("text=Fulya Kale").first().scrollIntoViewIfNeeded();
await p.waitForTimeout(900);
await p.screenshot({ path: `${shots}/results-fulya.png` });

// KVKK satırı
console.log("KVKK linki:", (await p.locator("a:has-text('KVKK Aydınlatma Metni')").count()) > 0);

// Form: gerçek kullanıcı simülasyonu (3 sn bekle → honeypot boş → kaydolmalı)
...
console.log("Form başarı ekranı:", (await p.locator("text=Talebiniz alındı").count()) > 0);
...
console.log("Footer TR numarası:", footerText.includes("+90"));
await b.close();
